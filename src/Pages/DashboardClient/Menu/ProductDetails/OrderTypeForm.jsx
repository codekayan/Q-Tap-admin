...
const OrderTypeForm = ({
  ...
  const toggleTypeForm = () => {
    // تحقق من وجود branchId قبل المتابعة
    if (!branchId) {
  ...
  const calculateTotalPrice = () => {
    const { calsubtotal, caltax, caldiscount } =
  ...
  // الخطوة 1: الحصول على branchId من localStorage
  useEffect(() => {
    const getBranchFromStorage = () => {
      // جرب عدة مفاتيح محتملة
      const savedBranchId =
      ...
      console.log("Branch ID:", savedBranchId);
      console.log("Shop ID:", savedShopId);

      if (savedBranchId) {
        setBranchId(savedBranchId);
        console.log("Branch ID set to:", savedBranchId);
      } else {
        console.error("No branch ID found in localStorage");
        toast.error("Please go back and select a branch first");
      }
  ...
  // الخطوة 2: تعديل دالة getBranchTable
  const getBranchTable = async () => {
    ...
    try {
      ...
      // جرب هاتين الطريقتين (أحدهما سيعمل)
      // الطريقة 1: GET مع query parameter
      const response = await axios.get(
        `${BASE_URL}tables?branch_id=${branchId}`,
        {
          headers: {
      ...
      // أو الطريقة 2: GET مع path parameter
      // const response = await axios.get(
      //     `${BASE_URL}tables/${branchId}`,
      //     {
      //         headers: {
      //             'Content-Type': 'application/json',
      //             'Authorization': `Bearer ${localStorage.getItem('Token')}`
      ...
      setTable(response.data.tables || response.data);
    } catch (error) {
  ...
  // الخطوة 3: استدعاء getBranchTable عندما يتغير branchId
  useEffect(() => {
    calculateTotalPrice();

    if (branchId) {
      console.log("Branch ID changed, fetching tables...");
      getBranchTable();
    }
  }, [branchId]);

  // الخطوة 4: استدعاء getBranchTable مرة أخرى عند تحميل المكون
  useEffect(() => {
    if (branchId) {
      getBranchTable();
    }
  ...
  if (isLoading) {
    return (
      ...
      >
        <Typography>Loading tables for selected branch...</Typography>
      </Box>
    );
  }
      ...
      >
        <AppBar position="sticky" color="inherit">
          <Toolbar>
            ...
            >
              ...
              <Typography
                variant="body2"
                sx={{ color: "#575756", fontSize: "10px", padding: "0px 5px" }}
              >
                Admin
              </Typography>
              <KeyboardArrowDownOutlinedIcon
                sx={{ color: "#575756", fontSize: "13px" }}
              />
            </Box>
          </Toolbar>
        </AppBar>
        ...
        <Box
          sx={{
            ...
            padding: "10px",
            marginBottom: "15px",
            display: selectedType === "Dine In" ? "block" : "none",
          }}
        >
          <Typography
            variant="body2"
            sx={{ fontSize: "10px", color: "#084298", fontWeight: "bold" }}
          >
            Table Selection Debug:
          </Typography>
          <Typography
            variant="body2"
            sx={{ fontSize: "9px", color: "#084298" }}
          >
            Current selectedTable value: <strong>{selectedTable}</strong>
          </Typography>
          <Typography
            variant="body2"
            sx={{ fontSize: "9px", color: "#084298" }}
          >
            Table options:{" "}
            {table.map((t) => `${t.name} (ID: ${t.id})`).join(", ")}
          </Typography>
        </Box>
        ...
        >
              ...
              <Box key={type.id}>
                <Button
                  ...
                  sx={{
                    ...
                    "&:hover": {
                      backgroundColor:
                        selectedType === type.type ? "#222250" : "#B2B2AD",
                    },
                    display: "flex",
                    flexDirection: "column",
                    alignItems: "center",
                ...
                <Typography
                  ...
                  sx={{
                    ...
                    color:
                      selectedType === type.type
                        ? theme.palette.orangePrimary.main
                        : theme.palette.text.dGray27_white,
          ...
          {/* تحذير إذا لم يكن branchId موجوداً */}
          {!branchId && (
            <Box
              sx={{
            ...
            >
              <Typography
                variant="body2"
                sx={{ color: "#856404", fontSize: "11px" }}
              >
                ⚠️ No branch selected. Please go back and select a branch to
                continue.
              </Typography>
            </Box>
          )}
                  ...
                  <FormControl fullWidth variant="outlined">
                    ...
                    >
                      ...
                      {table?.map((item, index) => (
                        <MenuItem
                          key={index}
                          value={String(item?.id)} // تأكد أن القيمة هي سلسلة نصية
                          sx={{
                            fontSize: "11px",
                            color: theme.palette.text.gray_white,
                          }}
                        >
                          {item?.name + " - size:" + item?.size} (ID: {item?.id}
                          )
                        </MenuItem>
                      ))}
                    </Select>
                ...
                <>
                  ...
                  <FormControl fullWidth variant="outlined">
                    ...
                    >
                      ...
                      {Governorates[Country.EGYPT].map((item, index) => (
                        <MenuItem
                          key={index}
                          value={item}
                          sx={{
        ...
        >
          <Box sx={{ width: "100%" }}>
            <Typography
              variant="h6"
              sx={{ fontSize: "9px", color: theme.palette.text.gray_white }}
            >
              {t("subTotal")}{" "}
              <span style={{ color: theme.palette.text.gray_white }}>
                {subTotal} EGP
              </span>
            </Typography>
          ...
          <Box sx={{ width: "100%" }}>
            <RadioGroup defaultValue="cash" onChange={handleChange}>
              <FormControlLabel
                sx={{ color: theme.palette.text.gray_white }}
                value="cash"
                control={
                  <Radio
                    size="small"
            ...
            <Button
              ...
              sx={{
                ...
                height: "30px",
                "&:hover": {
                  backgroundColor: branchId ? "#ef7d10" : "#949493",
                },
              }}
            >
              {t("placeOrder")}
              <DoneOutlinedIcon sx={{ fontSize: "18px", mr: 1 }} />
            </Button>
          </Box>
        </Box>
      </Box>
